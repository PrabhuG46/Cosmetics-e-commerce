...
function setupPaymentCards() {
    ...
    if (netBankingCard) {
        netBankingCard.addEventListener('click', () => handlePaymentSelection('Online'));
    }
}
...
function prefillUserDetails() {
    ...
    if (user) {
        ...
        if (addressField && user.address?.street) addressField.value = user.address.street;
        if (cityField && user.address?.city) cityField.value = user.address.city;
        if (stateField && user.address?.state) stateField.value = user.address.state;
        if (pincodeField && user.address?.pincode) pincodeField.value = user.address.pincode;
    }
}
